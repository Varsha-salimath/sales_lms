<template>
	<div v-if="quiz.data">
			...
			<div v-for="(question, qtidx) in questions">
				...
				>
					...
					<div
						...
						v-html="sanitizeHTML(questionDetails.data.question)"
					></div>
					<div v-if="questionDetails.data.type == 'Choices'" v-for="index in 4">
						...
						>
							...
							<span
								...
								v-html="sanitizeHTML(questionDetails.data[`option_${index}`])"
							>
							</span>
						</label>
						<div
		...
		<div v-else class="border rounded-lg p-12 text-center space-y-4">
			<div
			...
			<div v-else-if="quizSubmission.data.pass" class="space-y-3">
				<div class="flex justify-center">
					<CheckCircle class="w-12 h-12 text-green-500" />
				</div>
				<div class="text-2xl font-bold text-green-700">
					{{ __('Your Score: {0}%').format(Math.ceil(quizSubmission.data.percentage)) }}
				</div>
				<div class="text-ink-gray-7">
					{{ __('{0} out of {1} correct').format(quizSubmission.data.score, quizSubmission.data.score_out_of) }}
				</div>
				<div class="text-lg font-semibold text-green-600">
					{{ __('Excellent! You passed! Next lesson is now unlocked.') }}
				</div>
			...
			<div v-else class="space-y-3">
				...
				<div class="text-ink-gray-7 font-medium">
					{{ __('You need {0}% to pass this quiz.').format(quizSubmission.data.passing_percentage) }}
				</div>
				<div class="text-ink-gray-5">
					{{ __('Please review the lesson and try again.') }}
				</div>

				<div v-if="cooldownRemaining > 0" class="text-amber-600 font-medium mt-2">
					{{ __('You can retry in {0}').format(formatCooldown(cooldownRemaining)) }}
				</div>
				...
				<div class="flex justify-center gap-x-2 mt-4">
					...
					<div v-else class="text-sm text-ink-gray-5">
						{{ __('You have used all {0} attempts.').format(quizSubmission.data.max_attempts) }}
					</div>
					<Button
						v-if="!inVideo"
						@click="reviewLesson()"
	...
	quizSubmission.submit(
		{},
		{
			onSuccess(data) {
				...
				if (data.pass) {
					try {
						window.parent.postMessage({ type: 'lms-quiz-passed' }, '*')
					} catch (e) {}
				} else {
					try {
						window.parent.postMessage({ type: 'lms-quiz-failed' }, '*')
					} catch (e) {}
			...
			onError(err) {
				...
				if (errorTitle.includes('CooldownError')) {
					const errorMessage = err.messages?.[0] || err
					toast.error(__(errorMessage))
				}
			},
...
const canRetry = computed(() => {
	...
	const maxAttempts = quizSubmission.data.max_attempts
	if (!maxAttempts) return true
	return (quizSubmission.data.attempts_used || 0) < maxAttempts
})
...
const reviewLesson = () => {
	try {
		window.parent.postMessage({ type: 'lms-quiz-review-lesson' }, '*')
	} catch (e) {
		const pathname = window.location.pathname.split('/')
		const courseIdx = pathname.indexOf('courses')
		if (courseIdx > -1) {
			const lessonPath = pathname.slice(0, pathname.length).join('/')
...
const continueToNextLesson = () => {
	try {
		window.parent.postMessage({ type: 'lms-quiz-next-lesson' }, '*')
	} catch (e) {
