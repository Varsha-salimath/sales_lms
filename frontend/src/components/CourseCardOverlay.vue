	...
	<div class="border-2 rounded-md min-w-80 max-w-sm">
		...
		<div class="p-5">
			...
			<div v-if="!readOnlyMode">
				<div v-if="isAdmin" class="mb-3 space-y-2">
					<router-link
						:to="{
							name: 'CourseDetail',
					...
					>
						<Button variant="subtle" size="md" class="w-full">
							<template #prefix>
				...
				>
					<Button variant="subtle" class="w-full mt-2" size="md">
						...
						{{ __('Complete Feedback for Certificate') }}
					</Button>
				</router-link>
			</div>
			<div class="space-y-3">
...
const needsFeedback = computed(() => false)
...
const continueRoute = computed(() => {
	...
	return {
		...
		params: {
			...
			chapterNumber: data.current_lesson
				? data.current_lesson.split('-')[0]
				: 1,
			lessonNumber: data.current_lesson
				? data.current_lesson.split('-')[1]
				: 1,
		},
	}
})
