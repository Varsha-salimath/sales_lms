	...
	<div v-if="batch?.data" class="p-5">
		<div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
			...
			<NumberChartGraph
				...
				:value="totalAssessmentCount"
			/>
		</div>

		<div
			v-if="batch?.data?.live_course_assessments?.length"
			class="mb-6 border rounded-lg p-4"
		>
			<div class="text-sm font-semibold text-ink-gray-9 mb-2">
				{{ __('Live course assessments') }}
			</div>
			<p class="text-xs text-ink-gray-6 mb-3">
				{{
					__(
						'These are attached on the course and turned live for this batch from the course editor.'
					)
				}}
			</p>
			<ul class="space-y-2">
				<li
					v-for="row in batch.data.live_course_assessments"
					:key="row.name"
					class="flex items-center justify-between text-sm border rounded-md px-3 py-2"
				>
					<span>{{ row.title }}</span>
