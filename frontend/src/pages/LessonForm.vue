...
/** Each new save bumps this; older in-flight Editor.js saves must not overwrite or submit after a newer save. */
...
onMounted(() => {
	...
	setLessonPdfContext({ course: props.courseName })
...
const editLesson = createResource({
	...
	makeParams(values) {
		// Only send fields this form owns. Passing the full merged `lesson` object can
		// include read-only / stale keys and makes updates unreliable vs Editor.js output.
		return {
			doctype: 'Course Lesson',
			name: values.lesson,
			fieldname: {
				...
				include_in_preview: lesson.include_in_preview ? 1 : 0,
			},
		}
	},
})
...
const saveLesson = (e) => {
	...
	const myGen = ++saveGeneration
	editor.value
		.save()
		.then((outputData) => {
			if (myGen !== saveGeneration) return
			if (!outputData?.blocks) {
				if (showSuccessMessage) {
					toast.error(
						__(
							'Could not save lesson content. If this lesson has a table, try again after refreshing the page.'
						)
					)
				}
				return
			...
			return instructorEditor.value.save()
		})
		.then((outputData) => {
			if (myGen !== saveGeneration) return
			...
			if (!outputData?.blocks) {
				if (showSuccessMessage) {
					toast.error(
						__(
							'Could not save instructor notes. Try again after refreshing the page.'
						)
					)
				}
				return
...
const removeEmptyBlocks = (outputData) => {
	if (!outputData?.blocks) return outputData
	// Editor tools that persist no fields (save() returns {}) must still be kept.
	const keepWithEmptyData = ['paragraph', 'feedback']
	let blocks = outputData.blocks.filter((block) => {
		return (
			Object.keys(block.data).length > 0 ||
			keepWithEmptyData.includes(block.type)
		)
	})
...
#content .codex-editor {
	overflow: visible !important;
}

#content .codex-editor .ce-block__content {
	max-width: 100% !important;
	margin-inline: 0 !important;
}

#content .codex-editor .codex-editor__redactor {
	overflow: visible;
	display: flow-root;
	width: 100%;
}

.grid.md\:grid-cols-\[75\%\2c 25\%\] > .border-e {
	overflow-x: visible;
	overflow-y: auto;
}
