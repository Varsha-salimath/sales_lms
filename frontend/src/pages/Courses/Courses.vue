<template>
	...
	/>
	<ScormImportWizard
		v-if="showScormImportModal"
		v-model="showScormImportModal"
	/>
</template>
<script setup>
import {
...
import ScormImportWizard from '@/components/import/ScormImportWizard.vue'
...
const showScormImportModal = ref(false)
...
const isDemoCourse = (course) => {
	...
	return (
		title.includes('a guide to frappe learning') ||
		title.includes('frappe learning')
	)
}

const visibleCourses = computed(() =>
	(courses.data || []).filter((course) => !isDemoCourse(course))
)
...
const courseMenu = computed(() => {
	return [
		...
		{
			label: __('Import CRT Schedule'),
			icon: 'upload',
			onClick() {
				router.push({ name: 'SalesImport' })
			},
		},
		{
		...
		{
			label: __('Import SCORM Course'),
			icon: 'package',
			onClick() {
				showScormImportModal.value = true
			},
		},
	]
})
