<template>
	<div v-if="lesson.data" class="">
		...
		>
			...
			<div class="flex items-center gap-x-2">
				...
				<Tooltip
						...
						:text="nextLessonLocked ? __('Complete this lesson to proceed') : ''"
					>
						<Button
		...
		<div class="grid md:grid-cols-[70%,30%] h-[94vh]">
			<div v-if="lesson.data.lesson_locked" class="border-e">
				<div class="shadow rounded-md w-3/4 mt-10 mx-auto text-center p-4">
					...
					>
						{{ __('Go to Current Lesson') }}
					</Button>
				</div>
			</div>
			<div v-else-if="lesson.data.no_preview" class="border-e">
				<div class="shadow rounded-md w-3/4 mt-10 mx-auto text-center p-4">
					<div class="flex items-center justify-center mt-4 gap-x-2">
						<LockKeyholeIcon class="size-4 stroke-2 text-ink-gray-5" />
						<div class="text-lg font-semibold text-ink-gray-7">
			...
			>
				...
				>
					...
					<div
						v-if="user.data && lesson.data.membership && !allowEdit() && !lesson.data.progress"
						class="px-5 mt-8"
					>
						<div class="border rounded-lg p-6 text-center space-y-3 bg-surface-gray-1">
							<div v-if="hasQuiz && !quizPassed" class="space-y-2">
								<LockKeyholeIcon class="w-8 h-8 text-ink-gray-4 mx-auto" />
								<div class="text-ink-gray-7 font-medium">
									{{ __('Pass all quizzes to finish this lesson') }}
								</div>
							</div>
							<Button
								variant="solid"
					...
					<div
						v-if="lesson.data.progress"
						class="px-5 mt-8"
					>
						<div class="border rounded-lg p-4 text-center bg-green-50 border-green-200">
...
import {
	clearLessonPdfContext,
	setLessonPdfContext,
} from '@/utils/lessonPdfContext'
...
const setupLesson = (data) => {
	...
	setLessonPdfContext({ course: props.courseName, lesson: data.name })
...
const completeLesson = createResource({
	url: 'lms.lms.api.complete_lesson',
	makeParams() {
		return {
			lesson: lesson.data.name,
			course: props.courseName,
	...
	onSuccess(data) {
		...
		if (data.next_lesson) {
			router.push({
				...
				params: {
					courseName: props.courseName,
					chapterNumber: data.next_lesson.chapter_number,
					lessonNumber: data.next_lesson.lesson_number,
				},
			})
		}
	},
...
const goToCurrentLesson = () => {
	if (!lesson.data?.chapter_number) return
	router.push({
		name: 'Lesson',
		params: {
			courseName: props.courseName,
			chapterNumber: lesson.data.chapter_number,
			lessonNumber: lesson.data.lesson_number,
		},
	})
}

const checkQuizProgress = createResource({
	url: 'lms.lms.api.get_quiz_progress',
	makeParams() {
		return { lesson: lesson.data?.name }
	},
	onSuccess(data) {
		quizPassed.value = !!data
	},
})

const handleFeedbackNextLesson = () => {
	if (lesson.data?.next) {
		switchLesson('next')
	} else {
...
const handleQuizMessage = (event) => {
	...
	if (event.data.type === 'lms-quiz-passed') {
		quizPassed.value = true
	}
	if (event.data.type === 'lms-quiz-next-lesson') {
		switchLesson('next')
	}
	if (event.data.type === 'lms-quiz-review-lesson') {
		if (lessonContainer.value) {
			lessonContainer.value.scrollTo({ top: 0, behavior: 'smooth' })
		}
	}
}
...
const switchLesson = (direction) => {
	if (direction === 'next' && nextLessonLocked.value) return
	trackVideoWatchDuration()
	let lessonIndex =
		direction === 'prev'
...
const checkIfDiscussionsAllowed = () => {
	...
	if (hasQuiz.value && lesson.data?.name && user.data) {
		checkQuizProgress.submit()
	}
...
const nextLessonLocked = computed(() => {
	...
	if (lesson.data.progress) return false
	if (hasQuiz.value && !quizPassed.value) return true
	return false
})
...
.codex-editor .ce-block.ce-block--lms-float-start,
.codex-editor .ce-block.ce-block--lms-float-end,
.codex-editor .ce-block.ce-block--lms-float-start .ce-block__content,
.codex-editor .ce-block.ce-block--lms-float-end .ce-block__content {
	overflow: visible;
}
...
.codex-editor .ce-block.ce-block--lms-float-start {
	...
	margin-inline-end: var(--lms-img-ce-gap, 16px);
...
.codex-editor .ce-block.ce-block--lms-float-end {
	...
	width: fit-content;
	max-width: min(50%, 28rem);
	margin-inline-start: var(--lms-img-ce-gap, 16px);
...
.codex-editor .ce-block.ce-block--lms-float-end .ce-block__content {
	...
	max-width: min(50%, 28rem) !important;
...
.codex-editor .ce-block.ce-block--lms-float-start ~ .ce-block,
.codex-editor .ce-block.ce-block--lms-float-end ~ .ce-block {
	clear: none;
}

.codex-editor .ce-block.ce-block--lms-float-start + .ce-block .ce-paragraph,
.codex-editor .ce-block.ce-block--lms-float-end + .ce-block .ce-paragraph {
	min-height: 1.25rem;
}

.codex-editor .ce-block.ce-block--lms-block {
...
.codex-editor .ce-block.ce-block--lms-block .ce-block__content {
	display: block;
	max-width: 100% !important;
	width: 100% !important;
	margin-left: 0 !important;
...
.codex-editor .ce-block.ce-block--lms-center {
	float: none !important;
	clear: both;
	display: block;
	width: 100% !important;
...
.codex-editor .ce-block.ce-block--lms-center .ce-block__content {
	display: flex;
