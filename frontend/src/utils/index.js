...
import { ButtonBlock } from '@/utils/button'
import { FeedbackBlock } from '@/utils/feedback'
...
export const LESSON_UPLOAD_MAX_BYTES = 200 * 1024 * 1024

export function getFrappeErrorMessage(error) {
	if (!error) {
		return ''
	}
	if (typeof error === 'string') {
		try {
			return getFrappeErrorMessage(JSON.parse(error))
		} catch {
			return error
		}
	}
...
export function getServerErrorMessage(error, fallback = 'Something went wrong. Please try again.') {
	if (error?._server_messages) {
		try {
			const parsed = JSON.parse(JSON.parse(error._server_messages)[0])
			if (parsed?.message) {
	...
	if (error?.messages?.[0]) {
		return String(error.messages[0]).replace(/<[^>]+>/g, '').trim()
	}
	if (error?.message) {
		return String(error.message).replace(/<[^>]+>/g, '').trim()
	}
	return fallback
}
		...
		embed: {
			...
			config: {
				services: {
					...
					sharepoint: {
						regex: /^(https:\/\/[a-z0-9.-]*sharepoint\.com\/:[a-z]:\/\S+)$/i,
						embedUrl: '<%= remote_id %>',
						html: "<iframe style='width: 100%; height: 40rem; border: 1px solid #d7e4f7; border-radius: 16px; background: #fff' frameborder='0' allowfullscreen='true'></iframe>",
						id: (groups) => {
							const url = groups[0]
							if (/\/:v:\//i.test(url) || /[?&]action=/.test(url)) return url
							return `${url}${url.includes('?') ? '&' : '?'}action=embedview`
						},
					},
					googleForms: {
						regex: /^(https:\/\/(?:docs\.google\.com\/forms\/d\/e\/[A-Za-z0-9_-]+\/viewform\S*|forms\.gle\/[A-Za-z0-9]+))/i,
						embedUrl: '<%= remote_id %>',
						html: "<iframe style='width: 100%; height: 40rem; border: 1px solid #d7e4f7; border-radius: 16px; background: #fff' frameborder='0'></iframe>",
						id: (groups) => {
							const url = groups[0]
					...
					webframe: {
						regex: /^(https:\/\/(?:www\.)?(?:infinitylearn\.com|student\.infinitylearn\.com)\S*)/i,
						embedUrl: '<%= remote_id %>',
						html: "<iframe style='width: 100%; height: 36rem; border: 1px solid #d7e4f7; border-radius: 16px; background: #fff' frameborder='0'></iframe>",
						id: (groups) => groups[0],
					},
					codesandbox: {
				...
				},
				{
					...
					condition: () => {
						return (
							!forMobile &&
							userResource?.data &&
							(isAdmin() || userResource?.data?.is_student)
						)
					},
				},
			],
...
export const sanitizeHTML = (text) => {
	...
	text = DOMPurify.sanitize(decodeEntities(String(text)), {
		ALLOWED_TAGS: [
			'b',
			'br',
			'h1',
