...
def get_lessons(course: str, chapter: str = None, get_details: bool = True, progress: bool = False):
	...
	if chapter:
		if get_details:
			return get_lesson_details(chapter, progress=progress, course=course)
		else:
			return frappe.db.count("Lesson Reference", {"parent": chapter.name})

	for chapter in get_chapters(course):
		if get_details:
			lessons += get_lesson_details(chapter, progress=progress, course=course)
		else:
...
def get_lesson_details(chapter: dict, progress: bool = False, course: str = None):
	lessons = []
	...
	for row in lesson_list:
		...
		if progress:
			progress_row = frappe.db.get_value(
				"LMS Course Progress",
				{
					"member": frappe.session.user,
					"lesson": lesson_details.name,
				},
				["status", "scorm_progress"],
				as_dict=True,
			)
			lesson_details.progress = progress_row.status if progress_row else None
			lesson_details.is_complete = bool(
				progress_row and progress_row.status == "Complete"
			)
			lesson_details.scorm_progress = (
				progress_row.scorm_progress if progress_row else None
			)

		lessons.append(lesson_details)

	if progress and lessons and course:
		from lms.lms.lesson_locking import enrich_lesson_lock_status

		enrich_lesson_lock_status(course, lessons)
...
def get_course_card_details(courses: list) -> list:
	...
	if course_names:
		scorm_courses = set(
			frappe.get_all(
				"Course Chapter",
				filters={"course": ["in", course_names], "is_scorm_package": 1},
				pluck="course",
			)
		)
	...
	for course in courses:
		...
		course.has_scorm = course.name in scorm_courses
...
def get_course_details(course: str):
	...
	course_details.has_scorm = bool(
		frappe.db.exists("Course Chapter", {"course": course_details.name, "is_scorm_package": 1})
	)
...
def get_lesson(course: str, chapter: int, lesson: int) -> dict:
	...
	if membership and not lesson_details.include_in_preview:
		...
		if should_enforce_sequential_locking(course) and not is_lesson_unlocked(
			course, lesson_name, frappe.session.user
		):
			from lms.lms.lesson_locking import get_locked_lesson_redirect

			redirect = get_locked_lesson_redirect(course, frappe.session.user)
			redirect.update(
				{
					"title": lesson_details.title,
					"course_title": course_info.title,
...
def get_batch_details(batch: str):
	...
	batch_details.assessments = frappe.get_all(
		"LMS Assessment", {"parent": batch}, ["name", "assessment_name", "assessment_type"]
	)
	from lms.lms.course_assessment_visibility import get_live_course_assessments_for_batch
...
def get_country_code():
	...
	if not ip:
		return
	try:
		res = requests.get(f"https://ip-api.com/json/{ip}", timeout=3)
		data = res.json()
...
def get_assessments(batch: str) -> list:
	...
	for index, assessment in enumerate(assessments):
		if assessment.assessment_type == "LMS Assignment":
			assessments[index] = get_assignment_details(assessment, member)

		elif assessment.assessment_type == "LMS Quiz":
			assessments[index] = get_quiz_details(assessment, member)

		elif assessment.assessment_type == "LMS Programming Exercise":
			assessments[index] = get_exercise_details(assessment, member)

	from lms.lms.course_assessment_visibility import get_live_course_assessments_for_batch

	for course_assessment in get_live_course_assessments_for_batch(batch):
		existing = {(a.assessment_type, a.assessment_name) for a in assessments}
		if (course_assessment["assessment_type"], course_assessment["assessment_name"]) in existing:
			continue

		display_title = course_assessment.get("title")
		row = frappe._dict(
			{
				"name": course_assessment["name"],
				"assessment_type": course_assessment["assessment_type"],
				"assessment_name": course_assessment["assessment_name"],
				"from_course": True,
				"course": course_assessment.get("course"),
				"course_title": course_assessment.get("course_title"),
			}
		)
		if row.assessment_type == "LMS Assignment":
			row = get_assignment_details(row, member)
		elif row.assessment_type == "LMS Quiz":
			row = get_quiz_details(row, member)
		elif row.assessment_type == "LMS Programming Exercise":
			row = get_exercise_details(row, member)
	...
	return [row for row in assessments if row]


@frappe.whitelist()
def get_course_assessments(course: str) -> list:
	if not can_modify_course(course):
		frappe.throw(_("You are not authorized to view the assessments of this course."))

	from lms.lms.course_assessment_visibility import (
		ensure_visibility_rows,
		get_batches_for_course,
		get_visibility_map,
	)
	...
	assessments = frappe.get_all(
		...
		["name", "assessment_type", "assessment_name", "display_title", "idx"],
		order_by="idx",
	)

	batches = get_batches_for_course(course)
	visibility = get_visibility_map(course)

	for assessment in assessments:
		source_title = frappe.db.get_value(
			assessment.assessment_type, assessment.assessment_name, "title"
		)
		assessment.title = assessment.display_title or source_title
		assessment_batches = []
		assessment_visibility = visibility.get(assessment.name, {})
		for batch in batches:
			vis = assessment_visibility.get(batch.name, {})
			assessment_batches.append(
				{
					"batch": batch.name,
					"title": batch.title,
					"is_visible": vis.get("is_visible", 0),
				}
			)
		assessment.batches = assessment_batches

	return assessments


@frappe.whitelist()
def get_course_assessment_batches(course: str) -> list:
	if not can_modify_course(course):
		frappe.throw(_("You are not authorized to view batches for this course."))

	from lms.lms.course_assessment_visibility import ensure_visibility_rows, get_batches_for_course

	ensure_visibility_rows(course)
	return get_batches_for_course(course)


@frappe.whitelist()
def get_visible_course_assessments(course: str) -> list:
	from lms.lms.course_assessment_visibility import get_member_batches_for_course
	...
	batches = get_member_batches_for_course(member, course)
	if not batches:
		return []

	visible_assessments = frappe.get_all(
		...
		["course_assessment"],
		pluck="course_assessment",
	)
	visible_assessments = list(set(visible_assessments))
	if not visible_assessments:
		return []

	assessments = frappe.get_all(
		"LMS Assessment",
		{"name": ["in", visible_assessments]},
		["name", "assessment_type", "assessment_name", "display_title", "idx"],
		order_by="idx",
	)
...
def get_batches(filters: dict = None, start: int = 0, order_by: str = "start_date"):
	...
	if filters.get("student_all"):
		enrolled_batches = frappe.get_all(
			"LMS Batch Enrollment", {"member": frappe.session.user}, pluck="batch"
		)
		upcoming_batches = frappe.get_all(
			"LMS Batch",
			{"published": 1, "start_date": [">=", getdate()]},
			pluck="name",
		)
		batch_names = list({*(enrolled_batches or []), *(upcoming_batches or [])})
		if not batch_names:
			return []

		filters["name"] = ["in", batch_names]
		filters.pop("student_all", None)
...
def can_modify_batch(batch: str) -> bool:
	"""Moderator/System Manager: any batch. Others: only batches they instruct/evaluate."""
	if "System Manager" in frappe.get_roles(frappe.session.user):
		return True
	if has_moderator_role():
		return True
