...
class LMSQuizSubmission(Document):
	...
	def validate_cooldown(self):
		cooldown = cint(frappe.db.get_value("LMS Quiz", self.quiz, "retry_cooldown"))
		if not cooldown:
			return

		last_attempt = frappe.db.get_value(
			"LMS Quiz Submission",
			{"quiz": self.quiz, "member": frappe.session.user},
			"creation",
		...
		elapsed = time_diff_in_seconds(now_datetime(), last_attempt)
		...
		if elapsed < cooldown_seconds:
			...
			frappe.throw(
				_("Please wait {0}m {1}s before retrying this quiz.").format(minutes, seconds),
				CooldownError,
			)
...
class CooldownError(frappe.ValidationError):
	pass
