...
def submit_quiz(quiz: str, results: str):
	if frappe.session.user == "Guest":
		frappe.throw(_("Please login to submit the quiz."), frappe.PermissionError)
	...
	_ensure_quiz_access(quiz_details)
	...
	passed = percentage >= quiz_details.passing_percentage

	attempts_used = frappe.db.count(
		"LMS Quiz Submission",
		filters={"quiz": quiz, "member": frappe.session.user},
	)

	show_answers_now = quiz_details.show_correct_answers == "Always" or (
		quiz_details.show_correct_answers == "After Passing" and passed
	)

	cooldown_seconds = 0
	if not passed and cint(quiz_details.retry_cooldown):
		cooldown_seconds = cint(quiz_details.retry_cooldown) * 60
	...
	return {
		...
		"show_correct_answers": show_answers_now,
...
def check_answer(question: str, question_type: str, answers: str):
	"""Validate a single answer. Requires login; rate-limited to reduce answer-oracle abuse."""
	if frappe.session.user == "Guest":
		frappe.throw(_("Please login to continue."), frappe.PermissionError)

	# Soft rate limit: max 120 checks / minute / user (interactive quiz UX)
	cache_key = f"lms_quiz_check_answer:{frappe.session.user}"
	count = cint(frappe.cache().get_value(cache_key) or 0)
	if count >= 120:
		frappe.throw(_("Too many answer checks. Please wait a moment."))
	frappe.cache().set_value(cache_key, count + 1, expires_in_sec=60)

	# Question must belong to a quiz the user can access (course enrollment when linked)
	quiz_name = frappe.db.get_value("LMS Quiz Question", {"question": question}, "parent")
	if quiz_name:
		quiz_details = frappe.db.get_value(
			"LMS Quiz", quiz_name, ["name", "course", "lesson"], as_dict=True
		)
		if quiz_details:
			_ensure_quiz_access(quiz_details)
...
def _ensure_quiz_access(quiz_details) -> None:
	"""Staff always allowed; learners need enrollment when quiz is tied to a course."""
	roles = set(frappe.get_roles())
	if roles.intersection({"System Manager", "Moderator", "Course Creator", "Batch Evaluator"}):
		return

	course = quiz_details.course
	lesson = quiz_details.lesson
	if not course and lesson:
	...
	if not enrolled:
		frappe.throw(
			_("You must be enrolled in the course to take this quiz."),
